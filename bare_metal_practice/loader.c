...
void CopyLoadSegments(Elf64_Ehdr *ehdr) {
    Elf64_Phdr *phdr = (Elf64_Phdr*)((UINT64)ehdr + ehdr->e_phoff);
    for (Elf64_Half i = 0; i < ehdr->e_phoff; i++) {
        if (phdr[i].p_type != PT_LOAD) {
            continue;
        }

        UINT64 segm_in_file = (UINT64)ehdr + ehdr->e_phoff;
...
EFI_STATUS EFIAPI UefiMain(EFI_HANDLE image_handle, EFI_SYSTEM_TABLE *system_table) {
    ...
    if (EFI_ERROR(status)) {
        ...
        status = SaveMemoryMap(&memory_map, memory_map_file);
        if (EFI_ERROR(status)) {
            Print(L"failed to save memory map: %r\n", status);
            Halt();
        }
        status = memory_map_file->Close(memory_map_file);
        if (EFI_ERROR(status)) {
